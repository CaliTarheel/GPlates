		...
		const QString &markdown)
{
	...
	// and resolution fields above: a malformed schedule here must not disturb an otherwise-good
	// radius, since the two are unrelated concerns that happen to share a document. A missing
	// radius already returned above, so nothing below this point can retroactively invalidate it.
	//
