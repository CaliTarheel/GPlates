...
#include "view-operations/PolygonBooleanOperation.h"
...
namespace GPlatesQtWidgets
{
	namespace
	{
				...
				QAction *action)
		{
			...
			{
				// This helper is called from QAction::changed(). Updating the tooltip
				// emits changed() again, and recent Qt versions do not tolerate that
				// re-entrant mutation reliably.
				const QSignalBlocker signal_blocker(action);
...
GPlatesQtWidgets::ViewportWindow::ViewportWindow(
	...
	d_split_plate_operation_ptr(
			new GPlatesViewOperations::SplitPlateOperation(
				...
				get_application_state(),
				get_view_state())),
	d_polygon_boolean_operation_ptr(
			new GPlatesViewOperations::PolygonBooleanOperation(
				get_view_state().get_feature_focus(),
				get_application_state(),
				get_view_state())),
...
GPlatesQtWidgets::ViewportWindow::connect_tools_menu_actions()
{
	...
	QObject::connect(action_Polygon_Boolean_Operations, SIGNAL(triggered()),
			this, SLOT(handle_polygon_boolean_operation()));
	QObject::connect(action_Split_Plate, SIGNAL(triggered()),
			this, SLOT(handle_split_plate()));
...
void
GPlatesQtWidgets::ViewportWindow::handle_polygon_boolean_operation()
{
	const GPlatesViewOperations::PolygonBooleanOperation::Result result =
			d_polygon_boolean_operation_ptr->trigger(this);
	status_message(result.message);

	if (result.outcome == GPlatesViewOperations::PolygonBooleanOperation::OPERATION_ERROR)
	{
		QMessageBox::warning(this, tr("Polygon Boolean Operations"), result.message);
	}
	else if (result.outcome == GPlatesViewOperations::PolygonBooleanOperation::OPERATION_COMPLETED)
	{
		QMessageBox::information(this, tr("Polygon Boolean Operation Complete"), result.message);
	}
}
...
GPlatesQtWidgets::ViewportWindow::handle_split_plate()
{
	const GPlatesViewOperations::SplitPlateOperation::Result result =
			d_split_plate_operation_ptr->trigger(this);

	status_message(result.message);

	switch (result.outcome)
	{
	case GPlatesViewOperations::SplitPlateOperation::OPERATION_CANCELLED:
		break;
