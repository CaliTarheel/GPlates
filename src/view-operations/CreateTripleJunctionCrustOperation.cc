		...
		QWidget *parent_widget)
{
	...
	try
	{
		...
		// A band failing (e.g. "no recorded motion", often genuinely unphysical plate history
		// rather than a bug) no longer aborts the whole fill - the sides that do succeed are
		// still created, and the skipped one is named in the completion message so it can be
		// filled in separately (or the source data fixed) afterward.
		...
		for (unsigned int ridge_index = 0; ridge_index < ridges.size(); ++ridge_index)
		{
			...
			for (unsigned int side_index = 0; side_index < 2; ++side_index)
			{
				...
				if (!band.success)
				{
					skipped_bands << QObject::tr("Plate %1: %2").arg(sides[side_index]).arg(band.error);
					continue;
				}
		...
		if (polygon_count == 0)
		{
			throw std::runtime_error(skipped_bands.isEmpty()
					? QObject::tr("Existing OceanicCrust already fills the selected RRR interval.").toStdString()
					: QObject::tr("Every band failed, so nothing remains to fill: %1").arg(skipped_bands.join("; ")).toStdString());
		}
		...
		if (!skipped_bands.isEmpty())
		{
			completion_message += QObject::tr(
					" %1 side-crust piece(s) were skipped (often genuinely unphysical plate history,"
					" not a bug) and can be filled in separately once the source data is checked: %2")
