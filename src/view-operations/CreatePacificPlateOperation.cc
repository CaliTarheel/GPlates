...
namespace
{
	...
	public:
		PacificPlateUndoCommand(
				...
				const GPlatesModel::FeatureCollectionHandle::weak_ref &crust_collection,
				...
				const std::vector<GPlatesModel::FeatureHandle::non_null_ptr_type> &mor_features) :
			...
			d_crust_collection(crust_collection),
			d_crust_features(crust_features),
			d_mor_collection(mor_collection),
			d_mor_features(mor_features)
		{
		...
		virtual void redo()
		{
			if (!d_rotation_collection.is_valid() || !d_crust_collection.is_valid() ||
					!d_mor_collection.is_valid())
			{
				return;
			}
				...
				d_crust_features.begin(); feature_iter != d_crust_features.end(); ++feature_iter)
			{
				...
				{
					const GPlatesModel::FeatureCollectionHandle::iterator inserted =
							d_crust_collection->add(*feature_iter);
					*feature_iter = *inserted;
				}
			}
			for (std::vector<GPlatesModel::FeatureHandle::non_null_ptr_type>::iterator feature_iter =
				d_mor_features.begin(); feature_iter != d_mor_features.end(); ++feature_iter)
			{
				if (!(*feature_iter)->parent_ptr())
				{
					const GPlatesModel::FeatureCollectionHandle::iterator inserted =
							d_mor_collection->add(*feature_iter);
					*feature_iter = *inserted;
				}
			}
		...
		virtual void undo()
		{
			...
			for (std::vector<GPlatesModel::FeatureHandle::non_null_ptr_type>::reverse_iterator feature_iter =
				d_mor_features.rbegin(); feature_iter != d_mor_features.rend(); ++feature_iter)
			{
				if ((*feature_iter)->parent_ptr())
				{
	...
	private:
		...
		GPlatesModel::FeatureCollectionHandle::weak_ref d_crust_collection;
		...
		std::vector<GPlatesModel::FeatureHandle::non_null_ptr_type> d_mor_features;
	};
}
		...
		QWidget *parent_widget)
{
	...
	older_time_spin->setValue(project_older_bound ? *project_older_bound : current_time + fallback_interval);
	...
	form->addRow(QObject::tr("Default source:"), new QLabel(project_older_bound
			...
			: QObject::tr("50 My fallback (no Project Timestamp schedule active)"),
			&dialog));
	...
	GPlatesQtWidgets::ChooseFeatureCollectionWidget *collection_widget =
			new GPlatesQtWidgets::ChooseFeatureCollectionWidget(
					...
					&dialog);
	collection_widget->setTitle(QObject::tr("Destination crust collection"));
	collection_widget->set_help_text(d_preset_mor_destination.is_valid()
			? QObject::tr(
					"The new OceanicCrust polygons are written here. The rotation sequence is written to"
					" the rotation collection selected above, and the three new MidOceanRidge features go"
					" to the MOR destination set in the Ocean Crust Creation window.")
			: QObject::tr(
					"The new OceanicCrust polygons and its three new MidOceanRidge features are written"
					" here (no separate MOR destination was set in the Ocean Crust Creation window). The"
					" rotation sequence is written to the rotation collection selected above."));
	...
	try
	{
		...
		// Computed before the ordinary side-crust bands, and subtracted from each one's
		// candidate below, so every band terminates exactly at the same naturalized junction
		// edge the central crust is bounded by - rather than each being clipped independently
		// (the ordinary bands from the plates' own full ridge length, the central crust from
		// only the short stretch near the seed) and leaving a gap or overlap between them.
		const PacificPlateGeometry::BoundaryResult boundary = PacificPlateGeometry::compute_void_boundary(
				reconstructed_ridges, *d_void_seed, local_radius_spin->value(), naturalize_parameters);
		if (!boundary.success || !boundary.void_polygon)
		{
			throw std::runtime_error(boundary.error.toStdString());
		}
		...
		// An ordinary band failing (e.g. "no recorded motion", often genuinely unphysical
		// plate history rather than a bug) no longer aborts the whole plate birth - the
		// central crust and the new plate/MORs are the point of this tool, and are still
		// created; the affected side is skipped and named in the completion message so it can
		// be filled in separately (or the source data fixed) afterward.
		QStringList skipped_bands;
		// Bands are clipped against the void as well as existing crust and each other, so they
		// stop at the junction. The central crust is subtracted separately, against
		// crust_for_central (below), which does NOT include the void polygon itself - the void
		// is what is being subtracted *from*, not one of the things being subtracted.
		OceanCrustBandBuilder::polygon_seq_type crust_for_bands(existing_crust);
		crust_for_bands.push_back(*boundary.void_polygon);
		OceanCrustBandBuilder::polygon_seq_type crust_for_central(existing_crust);
		for (unsigned int ridge_index = 0; ridge_index < 3; ++ridge_index)
		{
			...
			for (unsigned int side_index = 0; side_index < 2; ++side_index)
			{
				...
				if (!band.success)
				{
					skipped_bands << QObject::tr("Plate %1: %2").arg(side_plates[side_index]).arg(band.error);
					continue;
				}
					...
					band.polygons.begin(); piece_iter != band.polygons.end(); ++piece_iter)
				{
					...
					crust_for_bands.push_back(*piece_iter);
					crust_for_central.push_back(*piece_iter);
				}
			}
		}

		const OceanCrustBandBuilder::Result uncovered_void =
				OceanCrustBandBuilder::subtract_existing_crust(**boundary.void_polygon, crust_for_central);
		if (!uncovered_void.success)
		{
			throw std::runtime_error(uncovered_void.error.toStdString());
		}
		...
		for (OceanCrustBandBuilder::polygon_seq_type::const_iterator component_iter =
				uncovered_void.polygons.begin(); component_iter != uncovered_void.polygons.end(); ++component_iter)
		{
			if ((*component_iter)->is_point_in_polygon(*d_void_seed))
			{
		...
		PacificPlateGeometry::Result geometry;
		geometry.success = true;
		geometry.existing_overlap_removed = uncovered_void.existing_overlap_removed;
		geometry.selected_endpoints = boundary.selected_endpoints;
		geometry.bounding_ridges = boundary.bounding_ridges;
		geometry.central_crust = seeded_components.front();
		geometry.inserted_vertex_count = boundary.inserted_vertex_count;
		geometry.maximum_seed_distance_degrees = boundary.maximum_seed_distance_degrees;
		geometry.actual_maximum_segment_length_km = boundary.actual_maximum_segment_length_km;
		...
		for (unsigned int edge_index = 0; edge_index < 3; ++edge_index)
		{
			...
			mor_features.push_back(MORFeatureBuilder::create_half_stage_mor(
		...
		// A destination set up front in the Ocean Crust Creation window takes priority; falling
		// back to the same collection as the new crust preserves the old combined behaviour for
		// anyone who has not set one.
		const GPlatesModel::FeatureCollectionHandle::weak_ref mor_collection =
				d_preset_mor_destination.is_valid() ? d_preset_mor_destination : geometry_collection;
		...
		std::unique_ptr<QUndoCommand> command(new PacificPlateUndoCommand(
				...
				geometry_collection, crust_features,
				mor_collection, mor_features));
		...
		if (!skipped_bands.isEmpty())
		{
			completion_message += QObject::tr(
					" %1 ordinary side-crust piece(s) were skipped (often genuinely unphysical plate"
					" history, not a bug) and can be filled in separately once the source data is"
					" checked: %2").arg(skipped_bands.size()).arg(skipped_bands.join("; "));
		}
